...
class Show extends Component
{
    ...
    public function mountComponent() {
        if (auth()->user()->is_admin == false) {
            $this->messages = \App\Models\Messenger::where('user_id', auth()->id())
                                                    ->orWhere('receiver', auth()->id())
                                                    ->orderBy('id', 'DESC')
                                                    ->get();
        } else {
            $this->messages = \App\Models\Messenger::where('user_id', $this->sender->id)
                                                    ->orWhere('receiver', $this->sender->id)
                                                    ->orderBy('id', 'DESC')
                                                    ->get();
        }
        $not_seen = \App\Models\Messenger::where('user_id', $this->sender->id)->where('receiver', auth()->id());
    ...
    public function SendMessage() {
        $new_message = new \App\Models\Messenger();
