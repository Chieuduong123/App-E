...
class QuizController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('user.quiz-list')->with('quiz_list', Quiz::join('quizz_questions', 'quizzes.id', '=', 'quizz_questions.quiz_id')
            ->distinct('quizzes.id')
    ...
    public function joinQuiz($id)
    {
        ...
        // if (auth()->user()->is_admin == false && coun8t(Result::where('user_id', auth()->user()->id)->get()) > 0) {
        //     return redirect()->back()->with('error', 'You already participated this quiz');
        // }
        ...
        return view('user.give-quiz')->with('quiz', Quiz::where('id', $id)->first())
            ->with('questions', QuizzQuestion::where('quiz_id', $id)->get())
